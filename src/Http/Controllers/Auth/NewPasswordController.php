...
use Dealskoo\User\Events\UserPasswordReset;
...
class NewPasswordController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return $status == Password::PASSWORD_RESET
            ? redirect()->route('user.login', [config('country.prefix') => request()->country()->alpha2])->withInput($request->only('email'))->with('status', __($status))
